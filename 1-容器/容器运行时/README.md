...
容器运行时是 Docker/K8s 等容器平台的核心，分层结构如下：
...
专注于容器生命周期管理，是 Docker 和 K8s 的核心组件。
...
ctr containers create nginx:alpine nginx1        # 创建容器
...
兼容 Docker CLI 的命令行工具，用法与 docker 完全一致。
