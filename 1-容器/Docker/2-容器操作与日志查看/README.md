...
| 方式 | 命令 | 特点 |
|------|------|------|
| exec（推荐） | docker exec -it xxx bash | 在运行中的容器执行新进程，exit 不会停止容器 |
| attach | docker attach xxx | 进入容器的主进程，exit 会停止容器 |
